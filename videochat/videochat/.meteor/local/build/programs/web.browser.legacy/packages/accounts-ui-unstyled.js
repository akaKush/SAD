...
var VALID_OPTIONS = new Set().add('passwordSignupFields').add('requestPermissions').add('requestOfflineToken').add('forceApprovalPrompt');
...
var displayName, getLoginServices, hasPasswordService, validateUsername, validateEmail, validatePassword;
module.link("./login_buttons.js", {
...
var login = function () {
  ...
  var usernameOrEmail = trimmedElementValueById('login-username-or-email'); // notably not trimmed. a password could (?) start or end with a space
...
var changePassword = function () {
  ...
  var oldPassword = elementValueById('login-old-password'); // notably not trimmed. a password could (?) start or end with a space
...
Template._configureLoginServiceDialog.events({
  ...
  'click #configure-login-service-dialog-save-configuration': function () {
    if (loginButtonsSession.get('configureLoginServiceDialogVisible') && !loginButtonsSession.get('configureLoginServiceDialogSaveDisabled')) {
      ...
      var inputs = [].slice.call( // Because HTMLCollections aren't arrays
